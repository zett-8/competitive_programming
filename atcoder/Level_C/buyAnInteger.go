...
func main() {
	t := uint64Array(strings.Split(readLine(), " "))
	A, B, X := t[0], t[1], t[2]
	var mx uint64
	for n := uint64(1); n <= uint64(math.Pow(10, 9)); {

		if X >= (A*n)+(B*uint64(len(strconv.FormatUint(n, 10)))) && n > mx {
			mx = n
			n++
		} else {
			break
		...
		if X >= (A*(n+1000))+(B*uint64(len(strconv.FormatUint(n+1000, 10)))) && n+1000 <= uint64(math.Pow(10, 9)) {
			n += 1000
		}

		if X >= (A*(n+100))+(B*uint64(len(strconv.FormatUint(n+100, 10)))) && n+100 <= uint64(math.Pow(10, 9)) {
			n += 100
		}
	}
